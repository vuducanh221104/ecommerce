    ...
    const handleClick = async () => {
        if (formik.isValid) {
            ...
            try {
                ...
                const dataAvatar = await axios.post('http://localhost:4000/api/admin/upload/uploadCloud', formData);
                formik.values.avatar = dataAvatar.data[0].path;

                //  SAVE DATA
                await axios.post(`http://localhost:4000/api/admin/user`, formik.values);
                formik.setValues({
                    fullname: '',
                    username: '',
                    avatar: '',
