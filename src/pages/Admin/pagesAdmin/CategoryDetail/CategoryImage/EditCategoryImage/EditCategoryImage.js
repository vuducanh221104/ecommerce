...
function EditCategoryImage() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            const response = await axios.get(`http://localhost:4000/api-image/image/image-category/${slugParam}`);
    ...
    const handleOk = () => {
        ...
        const fetchApi = async () => {
            await axios.delete(
                `http://localhost:4000/api-image/admin/image/edit/add-category-box-product/${slugParam}`,
            );
            try {
                setLoading(false);
            } catch (err) {
    ...
    const handleClick = () => {
        if (formik.isValid) {
            ...
            console.log(fileImageIndex);

            axios
                .post('http://localhost:4000/api/admin/upload/uploadCloud', formData)
                .then((response) => {
                ...
                .then(() => {
                    axios
                        .patch(
                            `http://localhost:4000/api-image/admin/image/edit/category-image/${slugParam}`,
                            formik.values,
                        )
                        .then((res) => {
                            setLoading(false);
                            ...
                            <>
                                ...
                                >
                                    ...
                                    <Form.Item label="Link">
                                        ...
                                        />
                                        <MessageFormikError
                                            formikErrorValue={formik.errors.images_theme?.[index]?.link}
                                        />
                                    </Form.Item>
                                </Form.Item>
                            </>
