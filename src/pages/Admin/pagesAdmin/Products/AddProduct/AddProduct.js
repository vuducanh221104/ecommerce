...
function AddProduct() {
    ...
    const handleClick = async () => {
        ...
        if (formik.isValid) {
            try {
                ...
                formik.values.image = mainImageResponse[0].path;
                console.log('Main image uploaded:', formik.values.image);

                // Second axios request - Add the product
                const productResponse = await axios.post('http://localhost:4000/api-product/product-add', {
                    image: formik.values.image,
                    name: formik.values.name,
                    price: formik.values.price,
                    price_discount: formik.values.price_discount,
                    prepay: formik.values.prepay,
                ...
                formik.values.images = imageUploadResponse.map((image) => image.path);
                console.log('Images uploaded:', formik.values.images);
                ...
                const colorImageUploadResponse = await uploadImage(
                    'http://localhost:4000/api/admin/upload/uploadCloud',
                    formDataImageColor,
                );

                colorImageUploadResponse.forEach((image, index) => {
                    formik.values.color[index].image = image.path;
                });

                console.log('Color images uploaded:', formik.values.color);

                // API PRODUCT DETAILS
                const productDetail = await uploadImage('http://localhost:4000/api-product/product-add-detail', {
                    name: formik.values.name,
                ...
                console.log('successful', productDetail);
                messageSuccess();
            } catch (error) {
                console.error(error);
                messageError();
    ...
    const editImageColor = (formikValuesData, nameValue, index) => {
            ...
            <WrapperContainer title={'Add New Product'}>
                ...
                >
                    ...
                    <Form.Item label="Price Prepay" className={cx('label-edit')}>
                        <Input name="prepay" id="prepay" value={formik.values.prepay} onChange={formik.handleChange} />
                        {formik.errors.prepay && <p className={cx('error-message')}>{formik.errors.prepay}</p>}
                    </Form.Item>
                        ...
                        <div>
                            <div style={{ marginTop: '55px' }}>
                                {(formik.values.color || []).map((item, index) => (
                                    <Form.Item label={`${index + 1}`} style={{ position: 'relative' }}>
