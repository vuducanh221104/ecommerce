...
function ProductList() {
    ...
    const handleDeleteCickOk = async (recordId, recordSlug) => {
        try {
            await axios.delete(`http://localhost:4000/api-product/product-delete/${recordId}`);
            await axios.delete(`http://localhost:4000/api-product/product-details-delete/${recordSlug}`);
            fetchApi();
            messageSuccess();
        } catch (error) {
            ...
            >
                ...
                >
                    <p>
                        Xóa:
                        {selectedId.map((item) => (
                            <div style={{ fontWeight: '700', fontSize: '1.5rem' }}> {item}</div>
                        ))}
                    </p>
                </Modal>
            </div>
