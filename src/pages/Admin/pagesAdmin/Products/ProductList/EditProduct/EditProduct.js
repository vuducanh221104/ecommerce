    ...
    const handleSubmit = async () => {
        ...
        if (formik.isValid && detailValid) {
            ...
            if (formik.values.image instanceof File) {
                const dataImage = await axios.post('http://localhost:4000/api/admin/upload/uploadCloud', formData);
                console.log(dataImage);
                formik.values.image = dataImage.data[0].path;
            }
            try {
                await axios.patch(`http://localhost:4000/api-product/product-update/${editProductId}`, {
